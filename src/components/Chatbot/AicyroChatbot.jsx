...
// IMPORTANT: Import Firebase logic
...
export default function AicyroChatbot() {
  ...
  // --- STATE TO PREVENT SPAMMING NOTIFICATIONS ---
  ...
  async function generateAndSendWebhook(data, timeText) {
    ...
    // --- TRIGGER SUCCESSFUL BOOKING NOTIFICATION ---
    triggerNotification(
      "Meeting Booked!",
  ...
  async function handleTextInput(e) {
    ...
    if (!hasTrackedConvo) {
      ...
      // --- TRIGGER NEW CHAT NOTIFICATION ---
      triggerNotification(
        "New Chat Started",
        `A visitor sent a message: "${val.length > 30 ? val.substring(0, 30) + "..." : val}"`,
      );
    }
    ...
    if (step === STEPS.AI_CHAT_MODE) {
      ...
      try {
        ...
        if (
          ...
          // --- TRIGGER NEW LEAD CAPTURED NOTIFICATION ---
          // Make sure we only notify once per session when basic contact info is gathered
          if (
            !hasNotifiedLead &&
            (updatedLeadData.name ||
              updatedLeadData.email ||
