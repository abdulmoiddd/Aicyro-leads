...
export default function Report({ onLogout }) {
  ...
  const [avgJobValue] = useState(1200);
  ...
  useEffect(() => {
    ...
    const unsubscribeAnalytics = onValue(analyticsRef, (snapshot) => {
      ...
      if (data) {
        setAnalytics({
          visitors: data.website_visitors || 0,
          opens: data.chatbot_opens || 0,
          convos: data.conversations_started || 0,
        });
      }
    });
  ...
  const reportData = useMemo(() => {
    ...
    switch (activeReport.id) {
      ...
      case "missed_opp":
        ...
        summary = [
          ...
          },
          {
            ...
            val: `$${(processedLeads.length * avgJobValue).toLocaleString()}`,
          },
        ];
        break;
      ...
      default:
        ...
        mapToChart(processedLeads);
        summary = [
          { label: "Website Visitors", val: scaledVisitors },
          { label: "Chatbot Opens", val: scaledOpens },
          { label: "Conversations Started", val: scaledConvos },
        ];
        break;
    }
    ...
    <main className="relative z-10 flex-grow w-full max-w-[1600px] mx-auto px-6 sm:px-12 py-8 fade-in printable-report-container">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 no-print">
        ...
        {onLogout && (
          <button
            onClick={onLogout}
            className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider bg-[var(--background)] hover:bg-red-500/10 text-[var(--foreground-muted)] hover:text-red-400 border border-[var(--border-color)] hover:border-red-500/30 px-5 py-2.5 rounded-lg transition-all"
          >
            Sign Out
          </button>
        )}
        ...
        <div className="lg:col-span-9 bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl flex flex-col overflow-hidden shadow-sm relative printable-area">
          ...
          <div className="px-8 py-6 border-b border-[var(--border-color)] bg-[var(--background)]/30 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-[var(--foreground-muted)] text-sm mt-1 print-text-black">
          ...
          <div className="flex-grow p-8 flex flex-col gap-8 overflow-y-auto custom-scrollbar">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
              {reportData.summary.map((card, i) => (
                <div
                  ...
                  className={`border rounded-xl p-5 print-border-black ${card.highlight ? "bg-[var(--lead-glow)] border-[var(--primary)]/30" : "bg-gradient-to-br from-[var(--background)] to-[var(--card-bg)] border-[var(--border-color)]"}`}
                >
                  <span
                    className={`text-[10px] font-bold uppercase tracking-widest ${card.highlight ? "text-[var(--primary)] print-text-black" : "text-[var(--foreground-muted)] print-text-black"}`}
                  >
                    {card.label}
                  </span>
                  <span className="block text-3xl font-black text-[var(--foreground)] mt-2 print-text-black">
                    {card.val}
                  </span>
                </div>
              ))}
            ...
            {/* Visual Data Preview (100% WORKING PURE REACT SVG ENGINE) */}
            <div className="bg-[var(--background)] border border-[var(--border-color)] rounded-2xl p-6 shadow-inner flex flex-col print-no-shadow relative">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-sm font-bold text-[var(--foreground)] print-text-black">
                  Trend Visualization
                </h3>
              ...
              <div className="w-full relative h-[260px]">
                ...
                >
                    ...
                    return (
                      <g key={i} className="opacity-10">
                        ...
                        <text
                          ...
                          textAnchor="end"
                          fill="var(--foreground)"
                          className="text-[10px] font-mono font-bold print-text-black"
                        >
                          {activeReport.id === "est_opp"
                            ? `$${Math.round((1 - ratio) * maxChartVal)}`
                            : Math.round((1 - ratio) * maxChartVal)}
                        </text>
                      </g>
                    );
                  ...
                  {reportData.chartBuckets.map((data, idx) => {
                    ...
                    // Dense configuration layouts (like 24h or 30 days) downsample ticks
                    ...
                    return (
                      ...
                      >
                        ...
                        <rect
                          ...
                          className="transition-all duration-500 ease-out shadow-[0_0_10px_var(--lead-glow)] print-bg-black"
            ...
            <div>
              <h3 className="text-sm font-bold text-[var(--foreground)] mb-4 border-b border-[var(--border-color)] pb-2 print-text-black">
              ...
              <div className="space-y-2">
                {reportData.tableData.length > 0 ? (
                  reportData.tableData.slice(0, 8).map((lead) => (
                    <div
                      key={lead.id}
                      className="flex flex-col sm:flex-row sm:justify-between sm:items-center text-xs p-3 bg-[var(--background)] border border-[var(--border-color)] rounded-lg hover:border-[var(--primary)]/50 transition-colors print-border-black"
                    >
                      <span
                        className="font-mono text-[var(--foreground-muted)] shrink-0 w-32 truncate print-text-black"
                        title={lead.id}
                      >
                        {lead.id.substring(0, 10)}...
                      </span>
                      <span className="text-[var(--foreground)] font-medium sm:w-1/4 truncate print-text-black">
                        {lead.name || "Unknown Visitor"}
                      </span>
                      <span className="text-[var(--foreground-muted)] sm:w-1/4 truncate print-text-black">
                        {lead.service_requested ||
                          lead.visitor_intent ||
                          "General Inquiry"}
                      </span>
                      <span className="text-[var(--foreground-muted)] sm:w-1/4 sm:text-center mt-2 sm:mt-0 print-text-black">
                        {new Date(lead.timestamp).toLocaleDateString()}
                      </span>
                      <span
                        className={`font-bold mt-2 sm:mt-0 text-right print-text-black ${lead.booking_status === "Meeting Booked" ? "text-green-400" : "text-[var(--primary)]"}`}
                      >
                  ...
                  <div className="text-center py-6 text-[var(--foreground-muted)] text-sm border border-dashed border-[var(--border-color)] rounded-xl bg-[var(--background)] print-text-black print-border-black">
      ...
      >
        <div className="bg-[var(--card-bg)] border border-[var(--border-color)] shadow-[0_10px_40px_rgba(0,0,0,0.3)] rounded-2xl p-4 pr-10 flex items-center gap-3 relative overflow-hidden backdrop-blur-xl">
          <div
            className={`absolute left-0 top-0 bottom-0 w-1.5 ${toast.type === "success" ? "bg-green-500" : "bg-red-500"}`}
          ></div>
          <div
            className={`w-8 h-8 rounded-full flex items-center justify-center ${toast.type === "success" ? "bg-green-500/20 text-green-400" : "bg-red-500/20 text-red-400"}`}
          >
            {toast.type === "success" ? (
              <svg
                className="w-4 h-4"
      ...
      <style
        ...
        @media print {
          ...
          .printable-report-container { background: white !important; padding: 0 !important; }
          .printable-area, .printable-area * { visibility: visible; }
          .printable-area { position: absolute; left: 0; top: 0; width: 100vw; height: auto; border: none !important; box-shadow: none !important; background: white !important; }
          .no-print { display: none !important; }
          .print-text-black { color: black !important; }
          .print-border-black { border-color: #ddd !important; }
          .print-border-solid { border-bottom-style: solid !important; opacity: 1 !important; border-color: #eee !important;}
          .print-bg-black { fill: #333 !important; stroke: #333 !important; }
