...
export default function HomeScreen({ onLogout }) {
  ...
  // --- NEW: Dynamic Average Job Value State ---
  ...
  useEffect(() => {
    ...
    const unsubscribeAnalytics = onValue(
      ...
      (snapshot) => {
        const data = snapshot.val();
        if (data) {
          setTotalWebsiteVisitors(data.website_visitors || 0);
          setTotalChatbotOpens(data.chatbot_opens || 0);
          setTotalConversationsStarted(data.conversations_started || 0);
        }
      },
      (error) => {
    ...
    // 4. --- NEW: Listen for Business Profile Data (Average Job Value) ---
    ...
    const unsubscribeProfile = onValue(
      ...
      (snapshot) => {
        ...
        if (data && data.avgJobValue) {
          // Parse the value to a number, fallback to 1200 if invalid
    ...
    return () => {
      ...
      unsubscribeConfig();
      unsubscribeProfile(); // Clean up new listener
    };
  }, []);
  ...
  // --- DYNAMIC SCALING FOR TOP KPIs (Matching Report.jsx Logic) ---
  const filterRatio =
  ...
  // Conversion Rates & Values (Updated to use scaledConvos)
  ...
  // Estimated Opportunity Value (Now dynamically uses firebase data)
  ...
  const highValue = highLeads * avgJobValue;
  const medValue = mediumLeads * Math.round(avgJobValue * 0.6);
  const pipelineSum = Math.max(highValue + medValue, 1);
  ...
  // Arrays for easy mapping of KPIs (Updated to use scaled variables)
  const topKpis = [
  ...
  return (
    <main className="relative z-10 mt-2 flex-grow w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-12 py-4 sm:py-6 fade-in-up">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 sm:gap-6 mb-8 items-stretch">
        ...
        {/* 3. Traffic by Day (New Vertical Bar Chart) */}
        <div className="bg-[var(--card-bg)] border border-[var(--border-color)] rounded-2xl p-5 sm:p-6 shadow-sm flex flex-col transition-all hover:-translate-y-1 hover:border-[var(--foreground-muted)]">
          <div>
            <h3 className="text-base sm:text-lg font-bold text-[var(--foreground)] mb-1">
